function InitVideo(options) {
	...
	function showCaptionContainerAndButton(obj) {
		obj.captionsBtnContainer.className = "media__captions-btn-container show";
		obj.captionsContainer.parentNode.parentNode.className = "has-captions";
		if (obj.isCaptionDefault) {
			obj.captionsContainer.className = "media__captions show";
	...
	function fullScreenStyles() {
		if (document.fullscreen || document.mozFullScreen || document.webkitIsFullScreen || document.msFullscreenElement) {
			var type = 'video';
			if (!obj.playerTypeVideo) {
				type = 'audio';
			}

			obj.fullScreenBtn.checked = true;
			//must apply other styles in container
			obj.container.className = "media__container media__container--" + type + " media__container--fullscreen";
			obj.controls.className = "media__controls media__controls--fullscreen";
			obj.captionsContainer.className = "media__captions js-fullscreen-captions";
		...
		// revert back to default styles
			obj.container.className = "media__container media__container--video";
			obj.controls.className = "media__controls";
			obj.captionsContainer.className = "media__captions";
	...
	if (obj.movie === undefined) {
		...
		obj.playerTypeVideo = false;
	}

	// Set up aria-label for Play button with the videoTitle option
	if ((typeof(options.videoTitle) === 'undefined') || (options.videoTitle === "")) {
		obj.playAriaLabel = FRAMEWORK.locale.settings.media.play;
	}
	else {
		obj.playAriaLabel = FRAMEWORK.locale.settings.media.playVideo + ", " + options.videoTitle;
		if(!obj.playerTypeVideo) {
			obj.playAriaLabel = FRAMEWORK.locale.settings.media.playAudio + ", " + options.videoTitle;
		}
	}
	...
	obj.controls.innerHTML = '<div class="media__progress-bar">' +
		'<progress class="media__progress" max="100" value="0"><span>0</span>% ' + FRAMEWORK.locale.settings.media.progress + '</progress>' +
		'<input id="progress' + obj.randomNum + '" class="media__progress-bar-marker" type="range" min="0" max="100" step="0.0001" value="0" />' +
		'</div>' +
		'<div class="media__progress-time">' +
			'<span class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.progressTime + '</span> <span class="media__duration">00:00</span>' +
		'</div>' +
		'<div class="media__playback-buttons">' +
			'<button class="media__restart"><span class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.restart + '</span></button>' +
			'<button class="media__rewind"><span class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.rewind + ' <span class="media__seconds">10</span> ' + FRAMEWORK.locale.settings.media.seconds + '</span></button>' +
			'<button class="media__play" aria-label="' + obj.playAriaLabel + '"><span class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.play + '</span></button>' +
			'<button class="media__pause media__hide"><span class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.pause + '</span></button>' +
			'<button class="media__forward"><span class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.forward + ' <span class="media__seconds">10</span> ' + FRAMEWORK.locale.settings.media.seconds + '</span></button>' +
		'</div>' +
		'<div class="media__volume-controls">' +
			'<div class="media__mute-btn-container">' +
				'<input class="media__mute u-hidden--visually" id="btnMute' + obj.randomNum + '" type="checkbox" />' +
				'<label id="labelMute' + obj.randomNum + '" for="btnMute' + obj.randomNum + '"><span class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.mute + '</span></label>' +
			'</div>' +
			'<div class="media__volume-slider">' +
				'<label for="volume' + obj.randomNum + '" class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.volume + ':</label><input id="volume' + obj.randomNum + '" class="media__volume" type="range" min="0" max="10" step="1" value="5" />' +
			'</div>' +
			'<div class="media__captions-btn-container media__hide">' +
				'<input class="media__btnCaptions u-hidden--visually" id="btnCaptions' + obj.randomNum + '" type="checkbox" />' +
				'<label for="btnCaptions' + obj.randomNum + '"><span class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.captions + '</span></label>' +
			'</div>' +
			'<div class="media__fullscreen-btn-container show">' +
			'<input class="media__btnFullScreen u-hidden--visually" id="btnFullscreen' + obj.randomNum + '" type="checkbox" />' +
			'<label for="btnFullscreen' + obj.randomNum + '"><span class="u-hidden--visually">' + FRAMEWORK.locale.settings.media.fullscreen + '</span></label>' +
		'</div>' +
		'</div>';
	...
	obj.btnPlay = obj.container.getElementsByClassName('media__play')[0];
	obj.btnPause = obj.container.getElementsByClassName('media__pause')[0];
	obj.btnRestart = obj.container.getElementsByClassName('media__restart')[0];
	obj.btnRewind = obj.container.getElementsByClassName('media__rewind')[0];
	obj.btnForward = obj.container.getElementsByClassName('media__forward')[0];
	obj.btnVolume = obj.container.getElementsByClassName('media__volume')[0];
	obj.btnMute = obj.container.getElementsByClassName('media__mute')[0];
	obj.progressBar = obj.container.getElementsByClassName('media__progress')[0];
	obj.progressBarMarker = obj.container.getElementsByClassName('media__progress-bar-marker')[0];
	obj.progressBarSpan = obj.progressBar.getElementsByTagName('span')[0];
	obj.captionsContainer = obj.container.getElementsByClassName('media__captions')[0];
	obj.captionsBtn = obj.container.getElementsByClassName('media__btnCaptions')[0];
	obj.captionsBtnContainer = obj.container.getElementsByClassName('media__captions-btn-container')[0];
	obj.duration = obj.container.getElementsByClassName('media__duration')[0];
	obj.txtSeconds = obj.container.getElementsByClassName('media__seconds');
	obj.fullScreenBtn = obj.container.getElementsByClassName('media__btnFullScreen')[0];
	obj.fullScreenBtnContainer = obj.container.getElementsByClassName('media__fullscreen-btn-container')[0];
	...
	obj.btnPlay.addEventListener('click', function() {
		obj.movie.play();
		obj.btnPlay.className = "media__play media__hide";
		obj.btnPause.className = "media__pause media__show-inline";
		obj.btnPause.focus();
	}, false);
	...
	obj.btnPause.addEventListener('click', function() {
		obj.movie.pause();
		obj.btnPlay.className = "media__play media__show-inline";
		obj.btnPause.className = "media__pause media__hide";
		obj.btnPlay.focus();
	}, false);
	...
	obj.movie.addEventListener('timeupdate', function() {
		...
		obj.progressBarMarker.value = obj.percent;
	...
	obj.progressBarMarker.addEventListener('change', function(e) {
		obj.pos = parseFloat(obj.progressBarMarker.value) / 100;
		obj.pos = obj.pos.toFixed(1);
	...
	obj.captionsBtn.addEventListener('click', function() {
		if (this.checked) {
			...
			obj.captionsContainer.className = "media__captions media__hide";
		}
